        ...
        if (it + 1) % args.chk_iter == 0:
            ...
            for b in range(len(test_holo_list)):
                ...
                if args.data_name == 'mnist':
                    ...
                    holo_batch, _ = next(iter(test_holo))
                    ...
                    d = d.to(device).float()
                    holo = holo_G(real_amplitude, real_phase, d).to(device).float().detach()

                    real_phase *= args.phase_normalize
                else:
                    real_amplitude, real_phase = next(test_gt)
                    holo = next(test_holo).to(device).float()

                real_amplitude = real_amplitude.to(device=device).float()
                real_phase = real_phase.to(device=device)

                real_amplitude = center_crop(real_amplitude, args.crop_size)
                real_phase = center_crop(real_phase, args.crop_size)
                holo = center_crop(holo, args.crop_size)
